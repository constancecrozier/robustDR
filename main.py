...
from sim_platform import (Simulation, Sim_Plot, get_casio_lmps)
...
for i in range(network.n_bus):
    for n in range(1):
        network.add_household(i,'data/'+hhs[i],#int(random.random()*len(hhs))],
...
for i in range(network.n_bus):
    for n in range(1):
        network.add_EV(i,n,'data/'+evs[ev_no],#int(random.random()*len(evs))],
                       datetime.datetime(2018,1,1))
        ev_no += 1
        if ev_no == len(evs):
            ev_no = 0
